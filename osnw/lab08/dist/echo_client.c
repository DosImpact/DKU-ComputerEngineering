...
int main(int argc, char **argv)
{
    ...
    int len, s;
    int sbyte, rbyte;
    struct cal_data sdata; // 보낼 데이터 구조체
    ...
    scanf("%s %d", tmp_str, &tmp_num);
    printf("client enter %s %d \n", tmp_str, tmp_num);
    memset((void *)&sdata, 0x00, sizeof(sdata));
    strcpy(tmp_str, sdata.char_data);
    sdata.num_data = (tmp_num);

    len = sizeof(sdata);
    sdata.num_data = htonl(sdata.num_data);
    ...
    if (write(server_sockfd, &sdata, sizeof(sdata)) <= 0) /* 입력 받은 데이터를 서버로 전송한다. */
    {
        perror("write error : ");
        return 1;
    }
    ...
    while (1)
    {
        ...
        printf("✔ data read from Server : %s", buf);
    }
    close(server_sockfd);
    return 0;
}
